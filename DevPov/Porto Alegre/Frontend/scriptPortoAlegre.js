...
async function carregarProgresso() {
  try {
    const resposta = await fetch(
      `http://localhost:3000/progresso/${idJogador}`
    );
    if (!resposta.ok) {
...
function comprarCafe() {
  if (dinheiro >= 300) {
    ...
    atualizarDinheiro();
    alert(
      "Você comprou a Máquina de Café Expresso! Agora você pode virar a noite codando no VSCode."
    );
    ...
    alert(
      "Você não tem dinheiro suficiente para comprar a Máquina de Café Expresso."
    );
  }
}
...
function verificarProgresso() {
  if (xp >= 10000) {
    alert(
      "Parabéns! Você atingiu um novo patamar e irá se mudar para São Paulo!"
    );
    window.location.href = "../../São%20Paulo/Frontend/SP.html"; // Redireciona para a fase 3
  }
}
